...
constexpr unsigned long features_to_use = 32;
...
struct lspe
{
    ...
    matrix_type const features( state_type const& s_ ) const
    {
        ...
        ans[20][0] = std::abs(ans[1][0] - ans[2][0]);
        ans[21][0] = std::abs(ans[2][0] - ans[3][0]);
        ans[22][0] = std::abs(ans[3][0] - ans[4][0]);
        ans[23][0] = std::abs(ans[4][0] - ans[5][0]);
        ans[24][0] = std::abs(ans[5][0] - ans[6][0]);
        ans[25][0] = std::abs(ans[6][0] - ans[7][0]);
        ans[26][0] = std::abs(ans[7][0] - ans[8][0]);
        ans[27][0] = std::abs(ans[8][0] - ans[9][0]);
        ans[28][0] = std::abs(ans[9][0] - ans[10][0]);
        //holes                                 // 1
        ans[29][0] = s_.holes();
        //highest column                        // 1
        ans[30][0] = *std::max_element( ans.begin()+1, ans.begin()+11 ); // highest column

        //more features
        ans[31][0] = *std::max_element( ans.begin()+21, ans.begin()+30 ); //max cliffs
    ...
    void step_in()
    {
        action_type const& best_action = select_action( game, theta.begin() );
        //auto const& best_reward = reward( game, best_action );
        //double const best_action_reward = best_reward.first;

        auto [best_action_reward, state_after_best_action] = reward( game, best_action );
        ...
        delta = gamma * lambda * delta + ( best_action_reward + gamma * std::inner_product( theta.begin(), theta.end(), psi_new.begin(), 0.0 ) - v );
        b += (v+delta) * psi;
        A += psi * psi.transpose();
    ...
    void operator()()
    {
        for ( size_type idx = 0; idx != training_set; ++idx )
        {
            ...
            std::ofstream ofs{ "./lspe_output_32features.txt", std::ofstream::out | std::ofstream::app };
    ...
    std::pair<unsigned long, state_type> reward( state_type const& s_, action_type const& a_ ) const
    {
        state_type s = s_;
        s.play_action( a_, false );

        return std::make_pair( s.lines_cleared - s_.lines_cleared, std::move(s) );
    }
