        ...
        for ( std::size_t k = n-1; ; --k )
        {
            for ( std::size_t its = 0; its < max_its; its++ )
            {
                ...
                for ( std::size_t j = l; j <= k-1; j++ )
                {
                    ...
                    f::for_each( v.col_begin(j), v.col_end(j), v.col_begin(j+1), [c, s]( value_type& v1, value_type& v2 ) { const auto vv1(v1); const auto vv2(v2); v1 = vv1*c + vv2*s; v2 = vv2*c - vv1*s; } );
                    ...
                    //defined in misc.hpp
                    f::for_each( u.col_begin(j), u.col_end(j), u.col_begin(j+1), [c, s]( value_type& v1, value_type& v2 ) { const auto vv1(v1); const auto vv2(v2); v1 = vv1*c + vv2*s; v2 = vv2*c - vv1*s; } );
