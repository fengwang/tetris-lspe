...
#define GENERATE_COMPLEX_MATRIX_MATH_UNARY_FUNCTION( f_name )  \
    ...
    { \
        ...
        f::for_each( m.begin(), m.end(), mm.begin(), []( T& t, std::complex<T> c ) { t = std:: f_name(c); } ); \
        return m; \
    }
...
#undef GENERATE_COMPLEX_MATRIX_ONLY_MATH_UNARY_FUNCTION
    ...
    const matrix<std::complex<T>, N> polar( const matrix<T,N,A>& mm, const matrix<T,N_,A_> nn )
    {
       ...
       f::for_each( mm.begin(), mm.end(), nn.begin(), m.begin(), []( const T _mm, const T _nn, std::complex<T>& m ) { m = std::polar(_mm, _nn); } );
       return m;
    }
    ...
    {
       matrix<std::complex<T>,N> m( mm.row(), mm.col() );
       f::for_each( mm.begin(), mm.end(), m.begin(), [nn]( const T _mm, std::complex<T>& m ) { m = std::polar(_mm, nn); } );
       return m;
    }
    ...
    {
       matrix<std::complex<T>,N> m( mm.row(), mm.col() );
       f::for_each( mm.begin(), mm.end(), m.begin(), [nn]( const T _mm, std::complex<T>& m ) { m = std::polar(nn, _mm); } );
       return m;
    }
