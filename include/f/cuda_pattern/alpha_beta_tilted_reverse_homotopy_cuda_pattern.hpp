...
namespace f
{
    ...
    struct cuda_pattern
    {
        ...
        std::vector<value_type>         ug_cache;
        ...
        std::function<value_type(value_type*)> make_reverse_function( value_type c, value_type lambda, value_type beta )
        {
            return [this, c, lambda, beta]( value_type* p )
            {
                ...
                auto ug_constrain = (*this).make_ug_constrain_function( beta );
                return zig( p ) + ug_constrain( p );
            };
        }
        ...
        std::function<value_type(value_type*)> make_ug_constrain_function( value_type beta )
        {
            return [this,beta]( value_type* p )
            {
                value_type res{0};
                size_type const offset = 2;
                for ( size_type index = offset; index != (*this).ug_cache.size(); ++index )
                {
                    value_type const diff = p[index] - ug_cache[index];
                    res += diff*diff;
                }
                return res*beta;
            };
        ...
        {
            cuda_assert( cudaMemcpy( reinterpret_cast<void*>(data.kt_factor), reinterpret_cast<const void*>(host_kt_factor), config.tilt_size*sizeof(value_type)*2, cudaMemcpyHostToDevice ) );
        ...
        {
            ug_cache.resize( config.ug_size*2 );
            unsigned long const offset = 2;
            std::copy( ug+offset, ug+config.ug_size*2, ug_cache.begin()+offset );
        }
