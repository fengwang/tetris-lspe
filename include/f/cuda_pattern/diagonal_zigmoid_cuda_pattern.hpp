#ifndef CDQDXWEFABFHKCFGRHIPUAPBMWGQEBOOGWHKMXSDPXMVSBVOUNCAKMYMVTUMJGLWLGTOJEJCM
#define CDQDXWEFABFHKCFGRHIPUAPBMWGQEBOOGWHKMXSDPXMVSBVOUNCAKMYMVTUMJGLWLGTOJEJCM
...
namespace f
{
    ...
    private:
        ...
        void update_I_diff( value_type* ug, value_type thickness, value_type c = 0.0 )
        {
            ...
            value_type const diag_ac = ug[config.tilt_size*2 + 1];
            value_type const diag_dc = ug[config.tilt_size*2 + 2];

            make_pattern_intensity_diff( data.ug, data.ar, data.diag, config.thickness, data.dim, data.I_exp, data.I_diff, config.column_index, data.cache, config.tilt_size, config.max_dim, c, data.I_zigmoid, diag_ac, diag_dc );
        }
...
#endif//CDQDXWEFABFHKCFGRHIPUAPBMWGQEBOOGWHKMXSDPXMVSBVOUNCAKMYMVTUMJGLWLGTOJEJCM
