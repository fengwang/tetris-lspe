#ifndef AUQHDYGDEFHSGJVDIFCOYWJCVTCJDJXEJGHIKPPCFGHVUHSXHMXLBKXULJATNQNCABKJOOKKX
#define AUQHDYGDEFHSGJVDIFCOYWJCVTCJDJXEJGHIKPPCFGHVUHSXHMXLBKXULJATNQNCABKJOOKKX
...
#include <f/pattern_to_image/pattern_to_image.hpp>
...
namespace f
{
    ...
    struct cuda_pattern
    {
        ...
        void dump_simulated_pattern( std::string const& path, std::string const& beam_path, std::string const& tilt_path, double rotation = 0.0 )
        {
            int current_id;
            cuda_assert( cudaGetDevice(&current_id) );
            if ( current_id != config.device_id ) cuda_assert( cudaSetDevice( config.device_id ) );

            matrix<double> diff_intensity{ config.max_dim, config.tilt_size };
            matrix<double> exp_intensity{ config.max_dim, config.tilt_size };
            
            device_to_host_copy_n( data.I_diff, diff_intensity.size(), diff_intensity.data() );
            device_to_host_copy_n( data.I_exp, exp_intensity.size(), exp_intensity.data() );

            matrix<double> sim_intensity = abs( exp_intensity - diff_intensity );

            matrix<long> beams;
            beams.load( beam_path );

            matrix<double> tilts;
            tilts.load( tilt_path );

            matrix<double> image;

            pattern_to_image( beams, tilts, sim_intensity, image, rotation );
...
#endif//AUQHDYGDEFHSGJVDIFCOYWJCVTCJDJXEJGHIKPPCFGHVUHSXHMXLBKXULJATNQNCABKJOOKKX
