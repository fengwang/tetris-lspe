#ifndef MLEVENBERG_MARQUARDT_HPP_INCLUDED_SDFOIU498FDSIUHVSKJHNC98Y4KJHFDSSIUHY4KJHSDFMJNBCKJHSASKJHSKJHCKJH4EU7KJSDFVNKJ
#define MLEVENBERG_MARQUARDT_HPP_INCLUDED_SDFOIU498FDSIUHVSKJHNC98Y4KJHFDSSIUHY4KJHSDFMJNBCKJHSASKJHSKJHCKJH4EU7KJSDFVNKJ
...
struct levenberg_marquardt
{
    ...
    typedef typename f::matrix<value_type>   matrix_type;
                 ...
                 )
    {
        ...
        for (;;)
        {
            ...
            if ( chi_square < old_chi && old_chi < chi_square * eps + chi_square  && chi_square < threshold ) break;

            if ( std::isnan(lambda) || std::isinf(lambda) ) break;
    ...
    {
        chi_square = f::inner_product( x_, _x, y_, sigma_, value_type(0),
                                          [f,a]( value_type x, value_type y, value_type sig )
                                          {
                                            const value_type fxa = f(x,a);
                                            if ( std::isnan(fxa) || std::isinf(fxa) )
                                            {
                                                assert(!"failed to evaluate chi^2 in f::levenberg_marquardt::update_chi_square!");
                                            }
    ...
    {
        chi_square = f::inner_product( x_, _x, y_, value_type(0),
    ...
    update_beta(IIX x_, IIX _x, A a, IIY y_, IIW sigma_, F f, IIP p_, IIP _p) 
    {
        ...
        {
            auto const tmp = f::inner_product( x_, _x, y_, sigma_, value_type(),
                                                  [a, f, p__]( value_type x, value_type y, value_type sig )
                                                  { return (y-f(x,a)) * ((*p__)(x,a)) / ( sig * sig ); } 
                                                );
            beta.push_back( tmp );
            if ( std::isnan(tmp) || std::isinf(tmp) )
                assert(!"failed to evaluate beta in f::levenberg_marquardt::update_beta!");
        }
    }
    ...
    update_beta(IIX x_, IIX _x, A a, IIY y_, F f, IIP p_, IIP _p) 
    {
        ...
        {
            auto const tmp = f::inner_product( x_, _x, y_, value_type(),
    ...
    update_alpha(IIX x_, IIX _x, A a, IIW sigma_, IIP p_, IIP _p) 
    {
        ...
        for ( std::size_t k = 0; k != M; ++k )
        {
            ...
            for ( std::size_t l = 0; l != N; ++l )
            {
                ...
                if ( std::isnan(mat[k][l]) || std::isinf(mat[k][l]) )
                    assert(!"failed to evaluate mat[][] in f::levenberg_marquardt::update_alpha!");
            }
        }
        ...
        for ( std::size_t k = 0; k < M; ++k )
            ...
            {
                alpha[k][l] = f::inner_product( mat.row_begin(k), mat.row_end(k), mat.row_begin(l), sigma_, value_type(),
                                                   [](value_type v1, value_type v2, value_type v3)
                                                   { return v1*v2*v3; } 
                                                 ); 
                if ( std::isnan(alpha[k][l]) || std::isinf(alpha[k][l]) )
                    assert(!"failed to evaluate alpha[][] in f::levenberg_marquardt::update_alpha!");
            }
    ...
    update_alpha(IIX x_, IIX _x, A a, IIP p_, IIP _p) 
    {
        ...
        for ( std::size_t k = 0; k < M; ++k )
            for ( std::size_t l = 0; l < M; ++l )
            {
                alpha[k][l] = f::inner_product( mat.row_begin(k), mat.row_end(k), mat.row_begin(l), value_type() ); 
                if ( std::isnan(alpha[k][l]) || std::isinf(alpha[k][l]) )
                    assert(!"failed to evaluate alpha[][] in f::levenberg_marquardt::update_alpha!");
            }
    ...
    update_delta_a()
    {
        delta_a.clear();
        matrix_type u, w, v;
        f::singular_value_decomposition( alpha, u, w, v );
        ...
        for ( auto v: delta_a )
        {
            if ( std::isnan(v) || std::isinf(v) )
                return 1;
                //assert(!"failed to solve equation in f::levenberg_marquardt::update_delta_a!");
        }
        return 0;
    }
