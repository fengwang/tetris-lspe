#ifndef DDJQYBSDCHATOTVHDLDIABAXAIIRATLGNVDMSFUEVBAOAJBHKPPJKTGWONMMHNIXKYTATMFUB
#define DDJQYBSDCHATOTVHDLDIABAXAIIRATLGNVDMSFUEVBAOAJBHKPPJKTGWONMMHNIXKYTATMFUB
...
namespace f
{
    ...
    // Usage: "cout << f::custom_delims<MyDelims>(x)".
    ...
    struct custom_delims_wrapper : public custom_delims_base
    {
            ...
            {
                return stream << ::f::print_container_helper<T, char, ::std::char_traits<char>, Delims>( t );
            }
            ::std::wostream& stream( ::std::wostream& stream )
            {
                return stream << ::f::print_container_helper<T, wchar_t, ::std::char_traits<wchar_t>, Delims>( t );
            }
...
inline std::basic_ostream<TChar, TCharTraits>& operator<<( std::basic_ostream<TChar, TCharTraits>& stream, const f::custom_delims<Delims>& p )
{
...
//template<typename T> using pp_sdelims = f::delimiters<T, char>;
//template<typename T> using pp_wsdelims = f::delimiters<T, wchar_t>;
...
namespace std
{
    ...
    inline basic_ostream<TChar, TCharTraits>& operator<<( basic_ostream<TChar, TCharTraits>& stream,
            const ::f::print_container_helper<T, TChar, TCharTraits, TDelimiters>& helper )
    {
        helper( stream );
        return stream;
    }
    ...
    inline typename enable_if<::f::is_container<T>::value, basic_ostream<TChar, TCharTraits>&>::type
    operator<<( basic_ostream<TChar, TCharTraits>& stream, const T& container )
    {
        return stream << ::f::print_container_helper<T, TChar, TCharTraits>( container );
    }
    ...
    {
        if ( ::f::delimiters<pair<T1, T2>, TChar>::values.prefix != NULL )
        {
            stream << ::f::delimiters<pair<T1, T2>, TChar>::values.prefix;
        }

        stream << value.first;

        if ( ::f::delimiters<pair<T1, T2>, TChar>::values.delimiter != NULL )
        {
            stream << ::f::delimiters<pair<T1, T2>, TChar>::values.delimiter;
        }

        stream << value.second;

        if ( ::f::delimiters<pair<T1, T2>, TChar>::values.postfix != NULL )
        {
            stream << ::f::delimiters<pair<T1, T2>, TChar>::values.postfix;
        }
...
/* The following macros allow us to write "template <TUPLE_PARAMAS> std::tuple<Args...>"
...
namespace std
{
    ...
    {
        if ( ::f::delimiters<::f::tuple_dummy_pair, TChar>::values.prefix != NULL )
        {
            stream << ::f::delimiters<::f::tuple_dummy_pair, TChar>::values.prefix;
        }

        ::f::pretty_tuple_helper<const tuple<Args...> &, tuple_size<tuple<Args...>>::value, TChar, TCharTraits>::print( stream, value );

        if ( ::f::delimiters<::f::tuple_dummy_pair, TChar>::values.postfix != NULL )
        {
            stream << ::f::delimiters<::f::tuple_dummy_pair, TChar>::values.postfix;
        }
...
template<typename T>
inline f::array_wrapper_n<T> pretty_print_array( const T* const a, size_t n )
{
    return f::array_wrapper_n<T>( a, n );
}
...
template <typename T> f::bucket_print_wrapper<T>
bucket_print( const T& m, typename T::size_type n )
{
    return f::bucket_print_wrapper<T>( m, n );
}

#endif//DDJQYBSDCHATOTVHDLDIABAXAIIRATLGNVDMSFUEVBAOAJBHKPPJKTGWONMMHNIXKYTATMFUB
