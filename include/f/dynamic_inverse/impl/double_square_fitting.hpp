#ifndef MDOUBLE_SQUARE_FITTING_HPP_INCLUDED_DSPOJASD2398ASFDKLJHSFDAKLJH4398YHASF
#define MDOUBLE_SQUARE_FITTING_HPP_INCLUDED_DSPOJASD2398ASFDKLJHSFDAKLJH4398YHASF
...
namespace f
{
    ...
    struct double_square_fitting
    {
        ...
        double_square_fitting() : target_residual_threshold_( value_type{1.0e-15} ), fitting_flag_( 1 ), max_iteration_( 1 ) {}

        void fit()
        {
            assert( y_.size() >= m_ );
            assert( x_real_.size() == x_imag_.size() );
            assert( m_ );
            assert( x_real_.size() );
            ...
            residual_ = double_square_ug_fitting( coef, intensity, result_matrix );

            a_.resize( m );
            a_[0] = value_type{1.0};
            std::copy( result_matrix.begin(), result_matrix.end(), a_.begin()+1 );
            ...
            do
            {
                ...
                std::cerr << "residual is " << residual_ << "\n";
...
#endif//_DOUBLE_SQUARE_FITTING_HPP_INCLUDED_DSPOJASD2398ASFDKLJHSFDAKLJH4398YHASF
