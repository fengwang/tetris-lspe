...
namespace f
{
    ...
    void eigen_hermitian_impl( const matrix<std::complex<T1>,D1, A1>& A, matrix<std::complex<T2>, D2, A2>& V, Otor o, const T eps = T( 1.0e-20 ) )
    {
        assert( A.row() == A.col() );
        ...
        for ( std::size_t i = 0; i != n;  ++i )
        {
            ...
            assert( offset < n+n ); 
            for_each( V.col_begin(i), V.col_end(i), VV.col_begin(offset), [](std::complex<T2>& c, T1 const r) { c.real(r); } ); 
            for_each( V.col_begin(i), V.col_end(i), VV.col_begin(offset)+n, [](std::complex<T2>& c, T1 const i) { c.imag(i); } ); 
